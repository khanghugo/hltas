...
fn strafe_type<W: Write>(type_: StrafeType) -> impl SerializeFn<W> {
    ...
    match type_ {
        ...
        MaxAccelYawOffset(_, _, _) => string("5"),
    }
}
    ...
    move |out: WriteContext<W>| match movement {
        ...
        Some(AutoMovement::Strafe(StrafeSettings { type_, dir })) => {
            ...
            let out = match dir {
                ...
                // Left or Right
                _ => {
                    // Write nothing for Left or Right because we will write the yawspeed in the next step.t
            ...
            let out = match type_ {
                ...
                StrafeType::MaxAccelYawOffset(start, target, accel) => tuple((
                    string(" "),
                    display(start),
                    string(" "),
                    display(target),
